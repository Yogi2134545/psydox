...
def _build_angle_prompt(product_desc: str, angle: dict) -> str:
    ...
    return (
        ...
        "keep the EXACT same color, colorway, pattern, print, graphic, logo, text, branding, "
        "material finish, texture, shape, silhouette, and proportions as in the reference image. "
        "If the reference shows a white shoe, the result must show a white shoe. "
        "If the reference shows a striped pattern, the result must show that exact same striped pattern. "
        "Only the camera angle changes — everything about the product itself is identical to the reference. "
